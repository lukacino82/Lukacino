...
struct SCStudyInterface{
 ...
 // Sierra (viz Trade Service Log uživatele): Buy/SellOrder jde přes Entry kontrolu - proti otevřené pozici jen s AllowOppositeEntryWithOpposingPositionOrOrders
 double sideOrder(s_SCNewOrder&o,int side){ bool opp=(pos!=0&&(pos>0)!=(side>0)); if(!opp) for(auto&kv:orders) if(kv.second.OrderStatusCode==SCT_OSC_OPEN&&stopSide[kv.first]!=side) opp=true;
   if(opp&&!AllowOppositeEntryWithOpposingPositionOrOrders){rejects++;return -5;} return market(o,side,false);}
